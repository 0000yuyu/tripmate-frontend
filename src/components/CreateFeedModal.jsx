...
export const CreateFeedModal = ({ isOpen, onClose, planUnitId, onSuccess, onFail }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append('planUnitId', String(planUnitId));
      formData.append('description', content.trim());
      formData.append('visibilityType', visibility);

      // 파일 배열들을 'originImages' 키값으로 추가
      images.forEach(img => {
        formData.append('originImages', img);
      });
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/60 backdrop-blur-md">
          ...
          >
            <div className="p-8 border-b border-gray-100 flex items-center justify-between">
              ...
              <button type="button" onClick={onClose} className="p-2 hover:bg-gray-100 rounded-full transition-all">
